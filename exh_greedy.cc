...
void exhaustive_search_rec(int i, int cp, int &mp, VI &cs, VI &cleft, VS &stations, const VI &ce, const VI &ne,
                           const VC &classes, VI &sol, VI &mkd, const double &start, const string &out)
{
    int C = cs.size();
        ...
        for (int j = 0; j < C; ++j)
        {
            if (not mkd[j])
            {
                ...
                if (int up = UPL(classes[cl].improvements, stations, ce, ne, i + 1 == C); up + cp < mp)
                    exhaustive_search_rec(i + 1, cp + up, mp, cs, cleft, stations, ce, ne, classes, sol, mkd, start, out);
...
void exhaustive_search(int C, const VI &ce, const VI &ne, const VC &classes, const string &out)
{
    ...
    VI sol = gen_sol(C, ce, ne, classes);
    VI mkd(C, false); // marked vector of sol
    ...
    exhaustive_search_rec(0, 0, mp, cs, cleft, stations, ce, ne, classes, sol, mkd, start, out);
}
