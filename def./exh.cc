...
int lower_bound_station(int i, int j, const VI &cs, const VI &cl, VI line, int ce_j, int ne_j, const MI &imp)
{
  int ones = ImprovementsLeft(j, cl, imp); // number of improvements (ones) left in that station
...
int lower_bound(int i, const VI &cs, const VI &cl, const VS &st, const Window &w, const MI &imp)
{
  int lb = 0;
  if (i >= 0)
  {
    ...
    for (int j = 0; j < M; ++j)
      lb += lower_bound_station(i, j, cs, cl, st[j].line, w.ce[j], w.ne[j], imp);
  }
  return lb;
}
